...
define([], function () {
   ...
   function newTexture(colors, SliderTrackOverride, SliderBorder) {
      ...
      for (let k = 0; k < colors.length; ++k) {
         let tint =
            typeof SliderTrackOverride != "undefined"
               ? SliderTrackOverride
               : colors[k];
         let bordertint =
            typeof SliderBorder != "undefined" ? SliderBorder : 0xffffff;
         ...
         for (let i = 0; i < width; i++) {
            ...
            if (
               innerPortion - position > 0 &&
               innerPortion - position < blurrate
            ) {
   ...
   function curveGeometry(curve0, radius) {
      ...
      function addArc(c, p1, p2, t) {
         ...
         let theta_1 = Math.atan2(
            vert[4 * p1 + 1] - vert[4 * c + 1],
            vert[4 * p1] - vert[4 * c]
         );
         let theta_2 = Math.atan2(
            vert[4 * p2 + 1] - vert[4 * c + 1],
            vert[4 * p2] - vert[4 * c]
         );
         ...
         for (let i = 1; i < divs; ++i) {
            vert.push(
               vert[4 * c] + radius * Math.cos(theta_1 + i * theta),
               vert[4 * c + 1] + radius * Math.sin(theta_1 + i * theta),
               t,
               1.0
            );
   ...
   SliderMesh.prototype.initialize = function (
      colors,
      radius,
      transform,
      SliderTrackOverride,
